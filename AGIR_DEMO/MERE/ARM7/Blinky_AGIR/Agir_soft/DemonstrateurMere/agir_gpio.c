...
void gpio_traitement(void)
{
	...
	if (gpio_get_value(PORT_CAPTEUR_MARGEUR_01,PIN_CAPTEUR_MARGEUR_01) == ON)
		can_emission(CAPTEUR_ON, ID_MODULE);
	
	// gestion des les pour le moteur
	if (gpio_get_value(PORT_BP_MOTEUR,PIN_BP_MOTEUR) == ON)
		set_led_margeur(MOTEUR,LED_ORANGE);
	else if (contacteur_0)
		set_led_margeur(MOTEUR,LED_GREEN);
	else 
		set_led_margeur(MOTEUR,LED_RED);
	
	// gestion des led pour le margeur
	if (gpio_get_value(PORT_BP_MARGEUR,PIN_BP_MARGEUR) == ON)
		set_led_margeur(MARGEUR_01,LED_ORANGE); 
	else if (contacteur_1)
		set_led_margeur(MARGEUR_01,LED_GREEN);		
	else
		set_led_margeur(MARGEUR_01,LED_RED);
}
