...
void isr_can_reception(void)__irq
{
	...
	{	
		switch(CAN1RDA)
		{
			case RQST_MODULE_0_NUM_1:
				rqst_moteur_on();
				break;

			case RQST_MODULE_0_NUM_2:
				rqst_moteur_off();
				break;

			case RQST_MODULE_0_NUM_3:
				rqst_margeur_on();	
				break;
			
			case RQST_MODULE_0_NUM_4:
				rqst_margeur_off();
				break;
			
			case RQST_MODULE_0_NUM_5:
				rqst_etat_systeme();
				break;
...
// recuperaion des etats des boutons 	
	...
	if ((gpio_get_value(PORT_CAPTEUR_MARGEUR_01,PIN_CAPTEUR_MARGEUR_01) == ON) && (etat == 0))
				etat += 0x10;
